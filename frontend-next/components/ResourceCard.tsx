...
export const ResourceCard = ({ title, description, icon, delay = 0 }: ResourceCardProps) => {
    return (
        <motion.div
            ...
            className="group relative w-full h-[320px] md:h-[400px] perspective-1000"
        >
            <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-white/10 backdrop-blur-xl border border-white/30 rounded-3xl shadow-xl transition-all duration-500 group-hover:shadow-[0_20px_50px_rgba(14,59,47,0.2)] overflow-hidden">
                ...
                <div className="h-full flex flex-col p-5 md:p-8 relative z-10">
                    <div className="mb-4 md:mb-6 transform group-hover:scale-110 transition-transform duration-500 origin-left">
                        <div className="w-12 h-12 md:w-16 md:h-16 bg-deepGreen/10 rounded-2xl flex items-center justify-center text-deepGreen">
                            {icon}
                        </div>
                    </div>

                    <h3 className="text-xl md:text-3xl font-bold text-deepGreen mb-3 md:mb-4 font-poppins group-hover:translate-x-2 transition-transform duration-300">
                        {title}
                    </h3>

                    <p className="text-charcoal/70 text-sm md:text-lg leading-relaxed mb-auto">
                        {description}
                    </p>
