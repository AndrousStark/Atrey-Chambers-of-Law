...
import AddToCalendarButton from '@/components/cms/ui/AddToCalendarButton';
...
function HearingDiaryInner() {
  ...
  // Inline update handler — saves a single field to DB
  const handleInlineUpdate = useCallback(async (caseId: string, field: string, value: string | null) => {
    try {
      await cmsCases.update(caseId, { [field]: value });
      // Update local state immutably
      setAllCases(prev => prev.map(c => c.id === caseId ? { ...c, [field]: value } : c));
                ...
                <table className="hearing-table w-full text-sm">
                  <thead>
                    <tr className="bg-[#1B2A4A] text-white text-left">
                      ...
                      <th className="px-3 py-3 font-semibold text-xs whitespace-nowrap text-center">Status</th>
                      <th className="px-3 py-3 font-semibold text-xs whitespace-nowrap text-center">Priority</th>
                      <th className="px-3 py-3 font-semibold text-xs whitespace-nowrap">Remarks</th>
                      <th className="px-3 py-3 font-semibold text-xs whitespace-nowrap text-center no-print">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 whitespace-nowrap">
                          {canEdit ? (
                            <input
                              type="text"
                              defaultValue={entry.case_.ndoh || ''}
                              onBlur={(e) => {
                                if (e.target.value !== (entry.case_.ndoh || '')) {
                                  handleInlineUpdate(entry.case_.id, 'ndoh', e.target.value || null);
                                }
                              }}
                              onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); }}
                              placeholder="DD.MM.YYYY"
                              className="h-7 w-[90px] px-1.5 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#1B2A4A] font-medium focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all"
                            />
                          ) : (
                            <span className="text-[#1B2A4A] font-medium text-xs">{formatDate(entry.hearingDate)}</span>
                          )}
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-center">
                          {canEdit ? (
                            <select
                              defaultValue={entry.case_.status}
                              onChange={(e) => handleInlineUpdate(entry.case_.id, 'status', e.target.value)}
                              className="h-7 px-1 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#333] focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all cursor-pointer"
                            >
                            ...
                            <span className="text-xs text-[#333]">{CASE_STATUS_LABELS[entry.case_.status] || entry.case_.status}</span>
                          )}
                        </td>
                        {/* Priority */}
                        <td className="px-3 py-2.5 text-center">
                          {canEdit ? (
                            <select
                              defaultValue={entry.case_.priority}
                              onChange={(e) => handleInlineUpdate(entry.case_.id, 'priority', e.target.value)}
                              className="h-7 px-1 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#333] focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all cursor-pointer"
                            >
                            ...
                            <span className="text-xs text-[#333]">{PRIORITY_LABELS[entry.case_.priority] || entry.case_.priority}</span>
                          )}
                        </td>
                        {/* Remarks */}
                        <td className="px-3 py-2.5 max-w-[200px]">
                          {canEdit ? (
                            <input
                              type="text"
                              defaultValue={entry.case_.remarks || ''}
                              onBlur={(e) => handleInlineUpdate(entry.case_.id, 'remarks', e.target.value || null)}
                              onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); }}
                              placeholder="Add remarks..."
                              className="h-7 w-full min-w-[120px] px-1.5 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#6C757D] focus:text-[#333] focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all"
                            />
                          ) : (
                            <span className="text-[#6C757D] text-xs" title={entry.case_.remarks || ''}>{truncate(entry.case_.remarks || '-', 40)}</span>
                          )}
                        </td>
                        {/* Actions */}
                        <td className="px-3 py-2.5 no-print">
                          <div className="flex items-center gap-1 justify-center">
                            <AddToCalendarButton caseData={entry.case_} size="sm" />
                            <Link
                              href={`/case-management/cases?view=${entry.case_.id}`}
                              target="_blank"
                              className="inline-flex items-center justify-center w-7 h-7 rounded hover:bg-[#4472C4]/10 text-[#6C757D] hover:text-[#4472C4] transition-colors"
                    ...
                    <div>
                      ...
                      {canEdit ? (
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); }}
                          placeholder="DD.MM.YYYY"
                          className="inline-block h-6 w-[80px] px-1 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#1B2A4A] font-semibold focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all"
                        />
                      ) : (
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-100">
                    {canEdit ? (
                      <input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); }}
                        placeholder="Add remarks..."
                        className="h-7 w-full px-1.5 rounded border border-transparent hover:border-[#4472C4]/30 bg-transparent focus:bg-blue-50/50 focus:border-[#4472C4]/30 text-xs text-[#6C757D] focus:text-[#333] focus:outline-none focus:ring-1 focus:ring-[#4472C4]/40 transition-all"
                      />
                    ) : (
                      entry.case_.remarks ? (
                        <p className="text-xs text-[#6C757D] line-clamp-2">{entry.case_.remarks}</p>
                      ) : null
                    )}
                  </div>
                </div>
