...
import { cmsCases, cmsCompliance, cmsFilings, cmsHearings, cmsScraper, cmsAuth } from '@/lib/cms-api';
import type {
  Case,
  ComplianceItem,
  FilingItem,
...
export default function CaseDetailPage() {
  ...
  const [sciToast, setSciToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null);
  ...
  useEffect(() => {
    if (!sciToast) return;
    const timer = setTimeout(() => setSciToast(null), 4000);
  ...
  const handleFetchFromSCI = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        setSciToast({ message: 'Case updated from SCI', type: 'success' });
        await fetchData();
      } else {
        setSciToast({ message: result.message, type: 'error' });
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to fetch from SCI';
      setSciToast({ message: msg, type: 'error' });
    } finally {
    ...
    <div className="flex flex-col gap-6">
      ...
      {sciToast && (
        <div
          ...
          style={{ backgroundColor: sciToast.type === 'success' ? GREEN : RED }}
        >
          {sciToast.type === 'success' ? (
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {sciToast.message}
          <button onClick={() => setSciToast(null)} className="ml-2 opacity-80 hover:opacity-100">
